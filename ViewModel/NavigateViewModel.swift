...
class NavigateViewModel: ObservableObject{
    @Published var navigate: [NavigateModel] = []
    ...
    func fetchData(){
        let url = URL(string: "https://npxvlreitylflxhdavqh.supabase.co/rest/v1/Tuwaiq?select=*")!
        ...
        let task = session.dataTask(with: request) { data, response, error in
            ...
            if let data = data {
                       do {
                           ...
                           let navigate = try decoder.decode([NavigateModel].self, from: data)
